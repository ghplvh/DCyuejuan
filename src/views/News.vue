        ...
        >
          <template slot-scope="scope">
            <img
              ...
              alt="新闻图片"
              @click="uploadImgPreview({url:scope.row.imageUrl})"
            >
          </template>
        </el-table-column>
        <el-table-column
...
export default {
  ...
  methods: {
    ...
    getNews() {
      this.loading = true
      return this.axios.post(API.NEWS_GETNEWS, { schoolCode: this.$store.state.adminInfo.teacherInfo.schoolCode }).then((res) => {
